...
public class SetupAction implements Callable<FullHttpResponse> {
  ...
  public FullHttpResponse call() throws Exception {
    ...
    if (null == require || "".equals(require)
        || (!require.equals(RtspRequestHelper.REQUIRE_VALUE_NGOD_R2))) {
      ...
      if(request.headers().get("OnDemandSessionId")!=null) {
    	  response.headers().set("OnDemandSessionId", request.headers().get("OnDemandSessionId"));
      }
      return response;
    }
    ...
    String sessionKey = RtspRequestHelper.keyFactory.createSessionKey();
    logger.debug("sessionKey --> " + sessionKey);
    RtspSession rtspSession = RtspRequestHelper.sessionAccessor.getSession(sessionKey, true);
    // save transport
