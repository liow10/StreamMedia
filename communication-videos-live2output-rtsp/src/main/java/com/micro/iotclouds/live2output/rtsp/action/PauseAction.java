...
import com.micro.iotclouds.live2output.rtsp.session.RtspSession;
import com.micro.iotclouds.live2output.rtsp.utils.DateUtil;
...
public class PauseAction implements Callable<HttpResponse> {
	...
	public HttpResponse call() throws Exception {
		...
		if (null == cseq || "".equals(cseq)) {
			...
			response.headers().set(Names.SERVER, RtspRequestHelper.SERVER);
		...
		if (null == require || "".equals(require) || (!require.equals(RtspRequestHelper.REQUIRE_VALUE_NGOD_R2))) {
		...
		// get session
		RtspSession rtspSession = RtspRequestHelper.sessionAccessor.getSession(sessionKey, false);
		if (null == rtspSession) {
		...
		final String RANGE = response.headers().get(RtspHeaderNames.RANGE);
		if (RANGE != null) {
			response.headers().set(RtspHeaders.Names.RANGE, RANGE);
		}
