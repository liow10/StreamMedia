...
import com.micro.iotclouds.communication.video.core.amf.Amf0Object;
import com.micro.iotclouds.communication.video.core.domain.Constants;
import com.micro.iotclouds.communication.video.core.entities.Role;
...
import com.micro.iotclouds.communication.video.core.utils.ByteToInputStream;
//import com.micro.iotclouds.communication.video.mp4.helpers.Client1;
//import com.micro.iotclouds.communication.video.mp4.helpers.Player;
//import com.micro.iotclouds.communication.video.mp4.helpers.SegmentVideoRecorder;
...
public class RtmpMessageHandler extends SimpleChannelInboundHandler<RtmpMessage> {
	...
	public void channelInactive(ChannelHandlerContext ctx) throws Exception {
		if (!normalShutdown && role == Role.Publisher) {
			Stream2 stream = streamManager.getStream(streamName);
			if (stream != null) {
...
//		}
	...
	private void handleMedia(ChannelHandlerContext ctx, RtmpMediaMessage msg) {
		...
		//StoreMp4MadiaHandler
		//StoreMp4MadiaHandler objStoreMp4MadiaHandler = new StoreMp4MadiaHandler(streamName,msg);
		//执行视频存储逻辑
		//StoreMp4MadiaHandler.getInstance().onMediaStream(msg.raw(), 0, msg.raw().length, false);
//		try {
//			SegmentVideoRecorder.recordSegment("rtmp://127.0.0.1:1935/live/ppp", "D:\\resources\\iot\\rtmp\\mp4\\eguid%03d.mp4", 10, 400, 300, 25);
...
//		new Thread(new SegmentVideoRecorder()).start();
		//new Thread(new Client1(ByteToInputStream.byte2Input(msg.raw()))).start();
//		String outputFile = "rtmp://localhost:1935/live/test2";
//		try {
//			//Player.frameRecord(ByteToInputStream.byte2Input(msg.raw()), outputFile, 1);
//			//Player.frameRecord(msg.raw(), outputFile, 1);
//			Player objPlayer = new Player();
//			objPlayer.frameRecord(msg.raw(), outputFile, 1);
//		} catch (org.bytedeco.javacv.FrameRecorder.Exception e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
//		} catch (Exception e) {
...
//		}
	...
	private void startPlay(ChannelHandlerContext ctx, Stream2 stream) {
		if (stream == null) {
			return;
		}
...
//		ctx.writeAndFlush(emptt);
	...
	private void createStream(ChannelHandlerContext ctx) {
		Stream2 s = new Stream2(streamName);
	...
	private void handleConnect(ChannelHandlerContext ctx, RtmpCommandMessage msg) {
		...
		if( ((Map) msg.getCommand().get(2)).get("objectEncoding") instanceof Integer) {
		...
		}else if( ((Map) msg.getCommand().get(2)).get("objectEncoding") instanceof Double) {
			Double clientRequestEncode = (Double) ((Map) msg.getCommand().get(2)).get("objectEncoding");
			if (clientRequestEncode != null && clientRequestEncode.intValue() == 3) {
				log.error("client :{} request AMF3 encoding but server currently doesn't support", ctx);
				ctx.close();
				return;
